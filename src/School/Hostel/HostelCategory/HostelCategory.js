...
const HostelCategory = ({navigation}) => {
  ...
  useEffect(() => {
    if (roomcategory) {
      setenquirylist(roomcategory);
      convertdata(roomcategory);
    }
  }, [roomcategory]);
  ...
  return (
    <View style={{flex: 1}}>
      <BackHeader title={'Hostel Category'} />
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('HosAddCategory')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
