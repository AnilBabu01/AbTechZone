...
const UpdateTranscashOnline = () => {
  ...
  const submit = () => {
    ...
    serverInstance('expenses/amounttransfer', 'put', data).then(res => {
      if (res?.status) {
        setloader(false);
  ...
  useEffect(() => {
    if (route.params?.data) {
      ...
      setaddDate(newdate);
      setExpensesAmount(route.params?.data?.Transfer_Amount?.toString());
      setComment(route.params?.data?.Comment);
      setExpensestype(route.params?.data?.Expensestype);
      setPayOption(route.params?.data?.Transfer_Mode);
    }
  }, []);

  return (
    <View>
      <BackHeader title={'Update Cash/Online Transfer'} />
      <ScrollView>
...
export default UpdateTranscashOnline;
