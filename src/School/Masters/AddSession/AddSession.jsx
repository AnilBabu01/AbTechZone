...
const AddSession = ({navigation}) => {
  ...
  const {Sessions, loading} = useSelector(state => state.GetSession);
  ...
  useEffect(() => {
    if (Sessions) {
      setenquirylist(Sessions);
      convertdata(Sessions);
    }
  }, [Sessions]);
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddSessio')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
