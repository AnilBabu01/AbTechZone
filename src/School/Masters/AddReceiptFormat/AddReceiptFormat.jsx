...
const AddReceiptFormat = ({navigation}) => {
  ...
  const {ReceiptFormat, loading} = useSelector(state => state.getReceiptFormat);
  ...
  useEffect(() => {
    if (ReceiptFormat) {
      setenquirylist(ReceiptFormat);
      convertdata(ReceiptFormat);

      console.log("format",ReceiptFormat);
  ...
  return (
    <View style={{flex: 1}}>
      <BackHeader title={'Add Receipt Format'} />
      <View style={styles.headerTitleContainer}>
        <View>
          <Text style={styles.secondaryTitle}>Receipt Master</Text>
        </View>
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddReceiFormat')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
...
export default AddReceiptFormat;
