...
const UpdateReceiptFormat = () => {
  ...
  const submit = () => {
    ...
    serverInstance('comman/receiptprefix', 'put', data).then(res => {
      if (res?.status) {
        setloading(false);
        Toast.show({
          type: 'success',
  ...
  useEffect(() => {
    if (route.params?.data) {
      ...
      setreceiptformat(route.params?.data?.receiptPrefix);
    }
  }, []);

  return (
    <View>
      <BackHeader title={'Update Designation'} />
      <ScrollView>
...
export default UpdateReceiptFormat;
