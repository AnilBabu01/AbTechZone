...
import {getcourse, GetClassSubject} from '../../../redux/action/commanAction';
...
const AddSubject = ({navigation}) => {
  ...
  const {Classsubject, loading} = useSelector(state => state.GetClassSubject);
    ...
    await Promise.all(
      Classsubject?.map((item, index) => {
        ...
        enquiryTableList[3].items.push({
          ...
          deleteUrl: 'comman/classsubject',
          allDetails: item,
          redirect: 'UpdateSubjec',
        });
      }),
    );
  ...
  useEffect(() => {
    if (Classsubject) {
      setenquirylist(Classsubject);
      convertdata(Classsubject);
    }
  }, [Classsubject]);
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <View style={styles.headerTitleContainer}>
        <View>
          <Text style={styles.secondaryTitle}>Subject Master</Text>
        </View>
      ...
      <DownEnquiry
        enquiry={Classsubject}
        filename={'ClasssubjectList'}
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddSubjec')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
