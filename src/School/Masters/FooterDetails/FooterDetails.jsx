...
import {GetFooterDetails, GetSession} from '../../../redux/action/commanAction';
...
const FooterDetails = ({navigation}) => {
  ...
  const {footerdetails, loading} = useSelector(state => state.GetFooterDetails);
    ...
    await Promise.all(
      footerdetails?.map((item, index) => {
        ...
        enquiryTableList[3].items.push({
          id: index,
          value: item?.ChairmanContactNo,
        });
        enquiryTableList[4].items.push({
          id: index,
          value: item?.PrincipalContactNo,
        });
        ...
        enquiryTableList[6].items.push({
          id: index,
          value: item?.Mapurl?.slice(0, 10),
        });
        enquiryTableList[7].items.push({
          id: index,
          value: item?.facilitycontent?.slice(0, 10),
        });
        ...
        enquiryTableList[8].items.push({
          ...
          deleteUrl: 'comman/designation',
          allDetails: item,
          redirect: 'UpdateFooter',
        });
      }),
    );
  ...
  useEffect(() => {
    if (footerdetails) {
      setenquirylist(footerdetails);
      convertdata(footerdetails);
    }
  }, [footerdetails]);
  ...
  return (
    <View style={{flex: 1}}>
      <BackHeader title={'Footer Details'} />
      <View style={styles.headerTitleContainer}>
        <View>
          <Text style={styles.secondaryTitle}>Footer Master</Text>
        </View>
      ...
      <DownEnquiry
        enquiry={footerdetails}
        filename={'FooterDetailsList'}
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddFooter')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
