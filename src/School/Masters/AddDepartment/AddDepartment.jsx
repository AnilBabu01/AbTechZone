...
const AddDepartment = ({navigation}) => {
  ...
  const {department, loading} = useSelector(state => state.getpart);
    ...
    await Promise.all(
      department?.map((item, index) => {
       ...
       enquiryTableList[2].items.push({
          ...
          deleteUrl: 'comman/department',
          allDetails: item,
          redirect: 'UpdateDepart',
        });
      }),
    );
  ...
  useEffect(() => {
    if (department) {
      setenquirylist(department);
      convertdata(department);
    }
  }, [department]);
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddDepart')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
