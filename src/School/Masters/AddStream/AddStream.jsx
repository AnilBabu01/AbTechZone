...
import {GetStream, getcourse} from '../../../redux/action/commanAction';
const AddStream = ({navigation}) => {
    ...
    await Promise.all(
      Stream?.map((item, index) => {
        ...
        enquiryTableList[4].items.push({
          ...
          deleteUrl: 'comman/stream',
          allDetails: item,
          redirect: 'UpdateStream',
        });
      }),
    );
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddStrea')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
