...
const AddRoute = ({navigation}) => {
  ...
  const {route, loading} = useSelector(state => state.GetRoute);
    ...
    await Promise.all(
      route?.map((item, index) => {
        ...
        enquiryTableList[3].items.push({
          id: index,
          value: item.routeName?.BusRentPermonth,
        });
        enquiryTableList[4].items.push({
          id: index,
          value: item?.StopName?.map((items, index) => {
            return <Text key={index}>{items?.StopName} , </Text>;
          }),
        });
        enquiryTableList[5].items.push({
          ...
          deleteUrl: 'transport/vehicleroute',
          Deleteicon: (
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <AnimatedFAB
        icon={'plus'}
        onPress={() => navigation.navigate('AddNewRoute')}
        label="Add"
        extended={false}
        color={Colors.white}
        style={[fabStyle]}
